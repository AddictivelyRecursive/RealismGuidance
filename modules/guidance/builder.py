...
from .hsi_guidance import FrozenMSTPlusPlus, HSIGuidance
...
def build_guidance_controller(
    *,
    ...
    mstpp_ckpt_path: str,
    hsi_curv_coeff: float = 1.0,
    hsi_edge_coeff: float = 0.25,
    hsi_interval: int = 2,
    hsi_input_size: Tuple[int, int] = (128, 128),
) -> GuidanceController:
    """
    ...
    # identity target comes from source image
    target_embed = arcface_model.inference(source_image_path)

    # pose / geometry target comes from target image
